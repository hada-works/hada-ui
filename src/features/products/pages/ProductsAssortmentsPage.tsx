  ...
  return (
    <div>
      ...
      >
        ...
        <span className={cn(
          "text-xs tabular-nums shrink-0 px-1 rounded",
      ...
      <div className="flex-1 overflow-auto px-4 py-3 space-y-4 text-sm">
        <div className="space-y-2.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground w-24 shrink-0">Mã SKU</span>
            <span className="font-mono font-semibold">{sku.code}</span>
          </div>
          <div className="flex items-center gap-2">
            <Building2 className="size-4 text-muted-foreground shrink-0" />
            <span className="text-muted-foreground w-24 shrink-0">Thương hiệu</span>
        ...
        <div className="border-t pt-3 space-y-2">
          <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-1">Danh mục</div>
          <div className="flex flex-wrap gap-1">
            {sku.groupPath.map((g, i) => (
              <span key={i} className="flex items-center gap-1">
                <Badge variant="outline" className="text-xs">{g}</Badge>
                {i < sku.groupPath.length - 1 && <ChevronRight className="size-3 text-muted-foreground" />}
              </span>
            ))}
          </div>
        </div>
        ...
        <div className="border-t pt-3 space-y-2">
          <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-1">Giá</div>
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="rounded border bg-muted/30 px-3 py-2">
              <div className="text-xs text-muted-foreground">Giá bán lẻ</div>
            ...
            <div className="rounded border bg-muted/30 px-3 py-2">
              <div className="text-xs text-muted-foreground">Đã bán 30d</div>
        ...
        <div className="border-t pt-3 space-y-2">
          <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-1">Hiệu quả</div>
          <div className="rounded border bg-muted/30 px-3 py-2.5 space-y-1.5">
        ...
        <div className="border-t pt-3 space-y-1.5">
          <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-1">Lịch sử</div>
          <div className="flex items-center gap-2">
            <AlignLeft className="size-3.5 text-muted-foreground shrink-0" />
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── Left: Category Tree ── */}
        <div className="w-60 shrink-0 border-r bg-muted/20 overflow-auto">
          <div className="px-3 py-3 border-b">
            <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">Danh mục sản phẩm</div>
        ...
        <div className={cn("flex flex-col flex-1 overflow-hidden", selectedSku ? "border-r" : "")}>
          ...
          {/* Table header */}
          <div className="flex items-center gap-3 px-4 py-2 border-b bg-muted/10 text-xs text-muted-foreground font-semibold uppercase tracking-wide shrink-0">
            <div className="flex-1">Sản phẩm</div>
            <div className="w-24 text-right shrink-0">Revenue 30d</div>
            <div className="w-16 text-right shrink-0">Margin</div>
            <div className="w-24 text-right shrink-0">Trạng thái</div>
          </div>
              ...
              filteredSkus.map(sku => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-sm font-semibold text-[hsl(var(--success))] tabular-nums w-24 text-right shrink-0">
          ...
          <div className="flex items-center justify-between px-4 py-2 border-t bg-muted/10 text-xs text-muted-foreground shrink-0">
            ...
            {selectedSku && (
              <button onClick={() => setSelectedSku(null)} className="text-primary hover:underline text-xs">
                Đóng chi tiết
              </button>
            )}
          </div>
