  ...
  const strategicKpis: { kpi: Parameters<typeof KpiCard>[0]["kpi"]; content: DrillContent }[] = [
    ...
    },
    {
      ...
      content: {
        ...
        status: (d.revVsPlan >= 0 ? "good" : "bad") as Status,
        rows: [
          { label: "Offline (in-store)", value: fmtVnd(d.totalRevenue * (1 - d.onlineRevPct / 100)), sub: `${(100 - d.onlineRevPct).toFixed(1)}% mix`, status: "neutral" as Status },
          { label: "Online / delivery",  value: fmtVnd(d.totalRevenue * d.onlineRevPct / 100), sub: `${d.onlineRevPct}% mix`, status: "good" as Status },
    ...
    },
    {
      ...
      content: {
        ...
        rows: [
          ...
          { label: "Rental cost % rev", value: `${d.rentalCostPct}%`, sub: "vs budget 8.0%", status: (d.rentalCostPct <= 8.0 ? "good" : "warn") as Status },
          { label: "Utilities % rev",   value: `${d.utilitiesPct}%`, sub: `vs budget 1.9%${d.utilitiesPct < 1.9 ? " — tiết kiệm" : ""}`, status: (d.utilitiesPct <= 1.9 ? "good" : "warn") as Status },
          { label: "D&A % rev",         value: `${d.dAndAPct}%`, sub: "on budget", status: "neutral" as Status },
        ] as DrillRow[],
  ...
  const expansionKpis: { kpi: Parameters<typeof KpiCard>[0]["kpi"]; content: DrillContent }[] = [
    ...
    },
    {
      ...
      content: {
        ...
        sections: [{
          ...
          rows: d.regions.map(r => ({
            label: r.name,
            value: `${r.pipeline} sites`,
            status: (r.pipeline >= 15 ? "good" : r.pipeline >= 8 ? "neutral" : "warn") as Status,
          })),
        }],
      },
    },
    ...
    },
    {
      ...
      content: {
        ...
        status: (Math.abs(d.headcountVsPlan) <= 2 ? "good" : "warn") as Status,
        rows: [
          { label: "Store staff",          value: d.headcountBreakdown.store.toLocaleString("vi"), sub: `${(d.headcountBreakdown.store/d.headcount*100).toFixed(0)}% HC`, status: "warn" as Status },
          { label: "DC / Logistics",       value: d.headcountBreakdown.dc.toLocaleString("vi"),    sub: `${(d.headcountBreakdown.dc/d.headcount*100).toFixed(1)}% HC`, status: "good" as Status },
          { label: "Head office",          value: d.headcountBreakdown.hq.toLocaleString("vi"),    sub: `${(d.headcountBreakdown.hq/d.headcount*100).toFixed(1)}% HC`, status: "warn" as Status },
          { label: "Turnover rate/tháng",  value: `${d.turnoverRatePct}%`, sub: "Benchmark ngành: 4.5% — tốt", status: (d.turnoverRatePct <= 4.0 ? "good" : "warn") as Status },
